/*
Description
You are given a directed acyclic graph with vertices and edges. There are no self-loops or multiple edges between anyYou should assign labels to all vertices in such a way that:
Labels form a valid permutation of length — an integer sequence such that each integer from to appears exactlIf there exists an edge from vertex to vertex then should be smaller than .
